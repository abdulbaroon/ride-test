...
export const ManageNotification = () => {
    ...
    useEffect(() => {
        if (userNotification) {
            ...
            setValue(
                "activityInviteEmail",
                userNotification.activityInviteEmail
            );
            setValue("activityInviteApp", userNotification.activityInviteApp);
            setValue(
                "activityUpdateEmail",
                userNotification.activityUpdateEmail
            );
            ...
            setValue(
                "activityDiscussionApp",
                userNotification.activityDiscussionApp
            );
            setValue(
                "activityRosterEmail",
                userNotification.activityRosterEmail
            );
            setValue("activityRosterApp", userNotification.activityRosterApp);
            setValue(
                "activityReminderEmail",
                userNotification.activityReminderEmail
            );
            setValue(
                "activityReminderApp",
                userNotification.activityReminderApp
            );
    ...
    return (
        <div className='w-11/12 mx-auto !max-w-[1320px] mt-28'>
            ...
            <div className='bg-white rounded-md border border-neutral-300 my-6'>
                        ...
                        <div key={index} className='flex mx-4 py-3 border-b'>
                            ...
                            <div className='w-1/2 flex font-semibold text-gray-500'>
                                <div className='w-1/2 text-lg'>
                                    {renderCheckbox(
                                        data.forEmail as keyof NotificationFormData
                                    )}
                                </div>
                                <div className='w-1/2'>
                                    {renderCheckbox(
                                        data.forApp as keyof NotificationFormData
                                    )}
                                </div>
                            </div>
                        </div>
