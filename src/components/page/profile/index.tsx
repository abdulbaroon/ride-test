...
export const ProfilePage = () => {
    ...
    const [imageSrc, setImageSrc] = useState<string>(
        "https://chasingwatts.com/pfImages/pfImg_35373.png"
    );
    ...
    const profile = async () => {
        if (userData.id) {
            ...
            if (getProfile.fulfilled.match(response)) {
                ...
                setValue(
                    "gender",
                    profileData.userGenderModel.userGenderID.toString()
                );
                setValue("homeLocation", profileData.homeLocation);
                setValue("rideStartRadius", profileData.defaultRadius);
                setValue(
                    "unitOfMeasure",
                    profileData.unitOfMeasureID === 1 ? true : false
                );
                setValue(
                    "rideType",
                    profileData.activityTypeModel.activityTypeID.toString()
                );
    ...
    const onSubmit = async (data: ProfileFormValues) => {
        ...
        const payload = {
            ...
            isDeleted: false,
            createdBy: userData?.id as number,
        ...
        try {
            if (userData.userProfile && userData.id) {
                ...
                if (userprofile.rejected.match(response)) {
                    ...
                    toast.success("Your profile has been saved! Let's ride.");
                    profile();
                }
            } else {
                setloading(true);
    ...
    const mapMemo = useMemo(
        () => (
            <MapBox center={[lng, lat]} initialZoom={11} circle={ride * 1600} />
        ),
        [lng, lat, ride]
    );
    return (
        <section className='w-full py-10 '>
            {!success ? (
                <div className='flex flex-col desktop:flex-row w-11/12  mx-auto !max-w-[1320px] gap-5 mt-[90px]'>
                    <div className='w-full desktop:w-3/4 bg-white rounded-md border border-neutral-300 py-10 px-10'>
                        ...
                        <div className='w-full flex justify-center flex-col items-center'>
                            <label
                                className=' relative flex justify-center flex-row cursor-pointer'
                                htmlFor='fileInput'>
                                <div className='relative w-36 h-36 overflow-hidden bg-gray-100 rounded-full mt-3 border'>
                                    <img
                                        src={`${imageSrc}?lastmod=${unique}`}
                                        alt='img'
                            ...
                            <label
                                htmlFor='fileInput'
                                className='bg-primaryButton w-fit py-1 mt-3 px-6 font-bold text-white rounded-lg '>
                                upload+
                            </label>
                            <input
                                type='file'
                        ...
                        <form onSubmit={handleSubmit(onSubmit)}>
                            <div className='flex flex-col tablet:flex-row gap-3 mt-10'>
                                <div className='w-full tablet:w-1/2'>
                                    <input
                                        ...
                                        className='py-[10px] w-full px-4 border rounded-lg'
                                ...
                                <div className='w-full tablet:w-1/2 relative'>
                                    <select
                                        ...
                                        className={`bg-white py-[10px] w-full px-4 border rounded-lg ${
                                            gender
                                                ? "text-black"
                                                : "text-gray-500"
                                        }`}>
                                ...
                                <div className='w-full tablet:w-1/2 flex justify-center items-center flex-col'>
                                    <div className='w-full'>
                                        <label className='text-xs text-gray-500'>
                                            Home location - search and select*
                                        </label>
                                        <input
                                        ...
                                        <div className='border flex justify-center items-center rounded-md py-[7px]'>
                                            <label className='flex justify-between items-center cursor-pointer'>
                                                ...
                                                <div className='flex flex-col'>
                                                    <span className='ms-3 text-xs font-medium text-[#244161]'>
                                                        Imperial
                                                    </span>
                                                    <span className='ms-3 text-xs font-medium text-[#244161]'>
                                                        mi | mph | ft
                            ...
                            <div className=' mt-6'>
                                ...
                                <p className='text-xs text-[#244161]'>
                                    Your profile will be limited only connected
                                    users.
                                </p>
                            </div>
                ...
                <div className='flex flex-col justify-center items-center gap-1 w-full tablet:w-7/12 desktop:w-5/12 mx-auto mt-10'>
                    ...
                    <div>
                        <h1 className='text-3xl tablet:text-4xl mt-5 text-secondaryButton'>
                            {getValues("firstName") || "steav"}, you’re all set.
                        </h1>
                    </div>
                    <div className='w-9/12 tablet:w-8/12 text-xs tablet:text-sm text-center text-secondaryButton'>
                        You will see any rides posted in {ride || 0} km radious
                        of {getValues("homeLocation")}{" "}
                    </div>
                    <button
                        type='button'
                        onClick={() => router.push("#")}
                        className='bg-secondaryButton py-[9px] w-9/12 tablet:w-8/12 px-6 font-bold text-white rounded-md mt-10'>
                        Take a Tour?
                    </button>
                    <button
                        type='button'
                        onClick={() => router.push("#")}
                        className='bg-primaryButton py-[9px] w-9/12 tablet:w-8/12 px-6 font-bold text-white rounded-md mt-3'>
                        LET&apos;S RIDE
                    </button>
                </div>
            )}
