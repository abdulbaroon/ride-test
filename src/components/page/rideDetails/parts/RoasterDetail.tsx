...
const RoasterDetail = () => {
    ...
    const rosterDetail = useSelector<RootState, RosterDetail[]>(
        (state) => state.rideDetail.rosterDetails
    );
    ...
    const filterRosters = (typeID: number) => {
        return (
            (isArray(rosterDetail) &&
                rosterDetail?.filter(
                    (data) =>
                        data?.responseTypeModel.responseTypeName ===
                        tabStatuses[typeID]
                )) ||
            []
        );
    ...
    return (
        <section id='roster'>
            <div
                className={`bg-white min-h-40 border rounded-lg sticky top-28 p-5`}>
                <div className='flex gap-2'>
                    {["Joined", "Maybe", "Nope"].map((tabName, tabIndex) => (
                        <button
                            key={tabIndex}
                            onClick={() => setSelectedTab(tabIndex + 1)}
                            className={`w-full py-[10px] rounded-lg uppercase shadow-sm hover:shadow-lg ${
                                tabIndex + 1 === selectedTab &&
                                "shadow-lg border text-primaryText"
                            }`}
                            style={{
                                backgroundColor: getBackgroundColor(
                                    tabIndex + 1
                                ),
                                borderColor: getBackgroundColor(tabIndex + 1),
                            }}>
                    ...
                    {rides.isGroup ? (
                        <>
                            {["A", "B", "C"].map((group) => (
                                <div key={group} className='mt-3'>
                                    {roster?.filter(
                                        (r) => r.groupLevel === group
                                    )?.length > 0 && (
                                        <div
                                            className='w-full bg-teal-200 mt-3 px-4  py-[6px] rounded-lg'
                                            style={{
                                                backgroundColor:
                                                    getBackgroundColor(
                                                        selectedTab
                                    ...
                                    {roster
                                        ...
                                        ?.map((data, index) => (
                                            <div
                                                ...
                                                />
                                                <Link
                                                    href={`/friend/${data.userProfileModel.userID}`}
                        ...
                        <>
                            <div
                                ...
                                style={{
                                    backgroundColor:
                                        getBackgroundColor(selectedTab),
                                }}>
                                {`${
                                    ["Yes", "Interested ", "No"][
                                        selectedTab - 1
