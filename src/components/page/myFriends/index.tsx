        ...
        {filteredFriends?.length > 0 ? (
          <div className="grid grid-cols-4 gap-2 mt-3">
            {filteredFriends?.map((data, i) => {
              return (
                <Link href={`/friend/${data.userProfileModelFollowing.userID}`} key={i}>
                <motion.div
                    // initial={{ scale: 1 }}
                    // whileHover={{ scale: 1.02 }}
                  className="bg-[#f4f5f9] m-1 flex  px-4 py-6 rounded-lg gap-3 hover:border hover:border-blue-500"
                >
                  <Avatar
                    borderWidth="0px"
                  ...
                  />
                  <div>
                    <h1 className="text-xl truncate overflow-hidden w-10/12 text-gray-700 font-bold">
                      {data.userProfileModelFollowing.firstName}{" "}
                      {data.userProfileModelFollowing.firstName}
                    </h1>
                    <p className="font-semibold  text-gray-600">
                      {data.userProfileModelFollowing.homeBaseCity},{" "}
                      {data.userProfileModelFollowing.homeBaseState}
                    </p>
                    <div className="mt-2">
                      {data.isConfirmed ? (
                        <Badge colorScheme="green">Conformed</Badge>
                      ) : (
