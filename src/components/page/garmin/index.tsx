...
import { authGarminUser, authStravaUser, checkGarminUser, checkRWGPSUser, deleteGarminUser, deleteRWGPSUser, getGarminCallbackUrl } from '@/redux/slices/externalServicesSlice'
            ...
            <div className='mt-28 !max-w-[1400px] w-11/12 mx-auto'>
                <div className="  py-6  ">
                    <h1 className=' text-gray-500 flex gap-1 items-center'>
                        ...
                        <IoIosArrowForward />
                        <Link href={"/account/profile"} className='hover:text-black'>Profile</Link>
                        <IoIosArrowForward />
                ...
                <div className='bg-white rounded-lg border '>
                    <p className='text-2xl font-bold p-4 border-b text-gray-600'>Garmin Connect</p>
                    <div className='p-4 border-b '>
                        <p className='text-xl font-bold text-gray-800 flex flex-col'>
                            <span>Connect to your Garmin account and create your group rides even easier!</span>
                            <span>When using RWGPS, Chasing Watts will auto load most of the ride details including the route!</span>
                        </p>
                        <p className='text-xl mt-5 text-gray-500'>Click below to manage your authentication.</p>
                    </div>
                    <div className='p-4'>
                        {auth ?
                            <button onClick={onAlertOpen} className='text-white text-lg font-bold py-2 px-4 bg-orange rounded-md uppercase'>REVOKE Garmin </button>
                            : <button onClick={handleCallbackUrl} className='text-white text-lg font-bold py-2 px-4 bg-orange rounded-md uppercase'>AUTHORIZE Garmin </button>}
                    </div>
                </div>
                <div className='mt-5 ms-1 text-gray-500 text-lg'>
                    <p>Not using Garmin? <Link className='text-primaryText' href={"https://www.garmin.com/register/"} target='_blank'> Register for a free account </Link> and start creating some great routes!</p>
                </div>
                <ReusableAlertDialog
                    isOpen={isAlertOpen}
