...
import { getRide } from '@/redux/slices/addRideSlice';
  ...
  useEffect(() => {
    if (data) {
      ...
      if (data?.activityRouteModel[0]?.gpxRoutePath) {
        (async () => {
          handleDownloadUrl(`${process.env.NEXT_PUBLIC_IMAGE_URL}${data?.activityRouteModel[0]?.gpxRoutePath}`)
        })();
      }
      if (data?.activityRouteModel[0]?.mapSourceID) {
        ...
        if (data?.activityRouteModel[0]?.mapSourceID === 5) {
          ...
          setValue("difficultygpx", data?.activityRouteModel[0]?.difficultyLevelID || "")
          ...
          setValue("difficulty", data?.activityRouteModel[0]?.difficultyLevelID || "")
          ...
          setValue("difficultylink", data?.activityRouteModel[0]?.difficultyLevelID || "")
          setValue("routeNamelink", data?.activityRouteModel[0]?.routeName)
          ...
          <form onSubmit={handleSubmit(handleSubmits)}>
            <Accordion defaultIndex={[0]} allowMultiple className='border bg-white rounded-xl'>
              <AccordionItem>
                <h2>
                  <AccordionButton>
                    <Box as='span' flex='1' textAlign='left' className='text-2xl font-bold'>
                  ...
                  <Tabs index={currentTab} onChange={(index) => setCurrentTab(index)}>
                    ...
                    <TabPanels>
                      <TabPanel>
                        {currentTab === 0 &&
                          <>
                            <RouteLink register={register} errors={errors} loading={loading} handleClick={handleClick} />
                            <RideRoute
                              type="link"
                              register={register}
                      ...
                      <TabPanel>
                        {currentTab === 1 &&
                          <><GPXFileUploader handleFile={handleFile} errors={errors} />
                            <RideRoute
                              type="gpx"
                              register={register}
                              ...
                              <div className="flex gap-0 tablet:gap-4 mt-3">
                                <div className="flex flex-col w-full relative">
                                  <select
                                    ...
                                    className={`bg-white border px-2 py-[6px]  ${errors.difficulty ? "input-error" : ""}`}
                                    defaultValue=""
                                  >
              ...
              <AccordionItem>
                ...
                <AccordionPanel pb={4}>
                  <RideDetails
                    data={data}
                    setData={(data: any) => setFinaldata({ ...finaldate, ...data })}
                    register={register}
                    errors={errors}
                    setValue={(name: string, value: any) => setValue(name as keyof FormData, value)}
                  />
                </AccordionPanel>
              </AccordionItem>
              ...
              <AccordionItem>
                ...
                <AccordionPanel pb={4}>
                  <AdditionalOptions register={register} errors={errors} data={data} setData={(data: any) => setFinaldata({ ...finaldate, ...data })} />
                </AccordionPanel>
              </AccordionItem>
            </Accordion>
            <div className='flex justify-center m-6'>
              <button type='submit' className='py-2 mx-auto px-8 bg-primaryDarkblue text-white' >
              {editLoading ? <CgSpinner className='mx-auto animate-spin w-6 h-6' /> : "Submit"}
              </button>
            </div>
          </form>
        </div>
