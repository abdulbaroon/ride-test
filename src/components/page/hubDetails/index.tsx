    ...
    <div className="  w-11/12  mx-auto !max-w-[1320px] gap-5 mt-[90px] ">
      ...
      >
        <div className="flex border rounded-lg overflow-hidden bg-white">
          <div className="w-[20%]">
            <img className="w-full h-full " src={hubImage.src} alt="" />
          </div>
          <div className="p-6 w-[30%] border-e">
            <p className="text-2xl font-bold text-gray-600">{hub.hubName}</p>
          ...
          <div className="p-6 w-[30%]">
            <p className="font-semibold  text-gray-600 flex gap-1 items-center">
              <GoLink />
              <Link className="text-primaryText" href={`${hub.hubUrl}`}>
                {hub.hubUrl}
              </Link>
            </p>
            {hub.hubPhone && (
              <p className="font-semibold items-center text-gray-600 flex gap-1">
                <FiPhone />
                <Link className="text-primaryText" href={`tel:${hub.hubPhone}`}>
                  {hub.hubPhone}
                </Link>
              </p>
            )}
      ...
      <div>
        {activeTab === "members" &&
          (member.length > 0 ? (
                      ...
                      <div className="p-6 flex flex-col items-center justify-center gap-1 ">
                        ...
                        <div className="mt-1">
                          <h1 className="text-xl font-bold text-center ">
                            {data.userProfileModel.firstName}{" "}
                            {data.userProfileModel.lastName}
                          </h1>
                          <p className="text-gray-600 text-center ">
                            {data.userProfileModel.homeBaseCity},{" "}
                            {data.userProfileModel.homeBaseCountry}
                          </p>
                        </div>
                      </div>
                    </div>
            ...
            <div className="min-h-28 w-full border relative mt-2 shadow-sm rounded-xl bg-white flex justify-center items-center">
            ...
            <div className="grid grid-cols-2 gap-6 mt-4">
              {isArray(formattedRide) &&
                formattedRide?.map((data) => (
                  <RideCard data={data} key={data.activityID} isLike={false} />
                ))}
            </div>
          ) : (
            <div className="min-h-32 flex-col w-full border relative mt-2 shadow-sm rounded-xl bg-white flex justify-center items-center">
              <h1 className="text-3xl font-semibold text-center text-gray-600">
                No upcoming rides
              </h1>
