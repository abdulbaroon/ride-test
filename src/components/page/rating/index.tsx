...
const StarIcon: FC<{ filled: boolean; half?: boolean }> = ({
    ...
    return (
        <FaStar
            className={`h-12 w-12 ${
                filled ? "text-blue-900" : "text-gray-300"
            }`}
        />
    );
};
...
export const RatingPage: FC<RatingPageProps> = ({ id }) => {
    ...
    const onSubmit: SubmitHandler<any> = async (data) => {
        if (rating === 0) {
            toast.error("Please rate the ride.");
        } else {
            const payload = {
    ...
    return (
        <div className='w-11/12 mx-auto !max-w-[1320px] mt-28'>
            <div
                className={` bg-white border border-neutral-300 rounded-md mb-6 mt-3 p-6 mx-auto ${
                    hasRated && "w-1/2"
                }`}>
                    ...
                    <div className='flex gap-8  '>
                        <div
                            className={` flex flex-col space-y-5 ${
                                hasRated ? "w-full" : "w-1/2"
                            }`}>
                            <div className='bg-neutral-100 border border-neutral-300 text-black p-4 rounded-md'>
                            ...
                            {!hasRated ? (
                                <>
                                    <p className='text-center font-bold w-2/3 items-center mx-auto'>
                                        How was your ride? Your feedback will
                                        help ride leaders create better rides.
                                    </p>
                                    ...
                                    <div className='flex justify-evenly items-center'>
                                        <button
                                            type='submit'
                                            className='w-2/5 bg-neutral-400 text-white py-2 rounded-md'
                                            onClick={() => router.back()}
                                            disabled={loading}>
                                            {loading ? (
                                        ...
                                        <button
                                            type='button'
                                            className='w-2/5 bg-primaryButton text-white py-2 rounded-md'>
                                            Submit Rating
                                        </button>
                                    </div>
                                </>
                                ...
                                <>
                                    <div className='flex justify-center flex-col items-center'>
                                        ...
                                        <p className='mt-4 mb-10 font-medium text-center'>
                                            {" "}
                                        ...
                                        <button
                                            onClick={() => router.back()}
                                            type='button'
                                            className='bg-primaryButton text-white py-2 px-6 rounded-md'>
                                            Go Back
                                        </button>
                                    </div>
                                </>
