...
interface QuickFilterProps {
    closeFilter: () => void
    handelFilterData:(data:any)=>void
}
        ...
        <div className='mt-6  border rounded-lg shadow-lg bg-white px-4 py-7 items-center relative'>
            ...
            <form onSubmit={handleSubmit(handleSubmits)}>
                <div className=' flex gap-x-6 '>
                    <div className='w-1/4'>
                        <input {...register("radius")} className='bg-white border px-2 py-[6px] w-full' type="number" placeholder='Radius' />
                    </div>
                    ...
                    <div className="w-1/4">
                        <input className='bg-white border px-2 py-[6px] w-full' {...register("rideName")} type="text" placeholder='Ride Name' />
                    </div>
                    <Tooltip hasArrow label='Add Filter' placement='top' bg='black'>
                        <button type='submit' className="text-2xl  text-white bg-primaryText  p-[7px] rounded-md h-fit "><TbFilter /></button>
                    </Tooltip>
                </div>
            </form>
