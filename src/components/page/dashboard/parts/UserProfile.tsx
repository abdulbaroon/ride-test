...
const UserProfile: React.FC<UserProfileProps> = ({userID, modifiedDate, name, username, level, location, radius, ridesJoined, distance, joules, joinedDate }) => (
  <div className=" w-full shadow-lg rounded-lg h-fit bg-white gap-3 overflow-hidden border">
    ...
    <UserProfileItem icon={<BsLightningChargeFill className="text-darkred text-xl" />} label="Level" value={level} link={`/friend/${userID}`} className="text-primaryDarkblue" />
    ...
    <UserProfileItem icon={<BsLightningCharge className="text-xl" />} label="Joules" value={joules} link='/points' className="text-primaryButton" />
...
const UserProfileItem: React.FC<UserProfileItemProps> = ({ icon, label, value, link, className = "" }) => (
