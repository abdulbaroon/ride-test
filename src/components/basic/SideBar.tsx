            ...
            <div className="mt-7 flex gap-2 border-b pb-2">
              ...
              <div className="text-gray-500 text-xs">
                ...
                <p className="text-bold">
                  {profile.homeBaseCity}, {profile.homeBaseCountry}
                </p>
                <p className="text-bold">
                  Radius: {profile.defaultRadius} miles
                </p>
                <p className="text-bold">{email && email.email}</p>
              </div>
            </div>
            <div>
              {sideBarData.map(({ name, icon, link, border }, index) => {
                return (
                  <div key={index} onClick={onClose}>
                    ...
                    <div
                      className={` ${border && "border-b h-1 w-full my-1 "} `}
                    ></div>
                  </div>
                );
              })}
