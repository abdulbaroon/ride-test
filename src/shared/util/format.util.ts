...
export const calculateTime = (distance: number | undefined, speed: number|any) => {
  if (distance) {
...
export const formatRideData = (item: Item): FormattedRideData => {
  ...
  return {
    ...
    difficultyLevelDescription:
      item?.activityRouteModel?.[0]?.difficultyLevelModel.levelDescription,
    ...
    activityDateTime: item.activityDate,
